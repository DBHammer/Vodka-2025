...
CREATE OR REPLACE FUNCTION vodka_cid_from_clast(
	in_c_w_id integer,
	in_c_d_id integer,
...
DECLARE
	...
	FOR
    	SELECT c_id FROM vodka_customer
	    WHERE c_w_id = p_w_id
	      AND c_d_id = p_d_id
	      AND c_last = p_c_last
	    ORDER BY c_first;
...
CREATE OR REPLACE FUNCTION vodka_proc_new_order(
    IN in_w_id integer,
    IN in_d_id integer,
    IN in_c_id integer,
...
CREATE OR REPLACE FUNCTION vodka_proc_payment(
	IN in_w_id integer,
	IN in_d_id integer,
...
BEGIN
	...
	IF in_c_last IS NOT NULL THEN
	    in_c_id = vodka_cid_from_clast(in_c_w_id, in_c_d_id, in_c_last);
	END IF;
...
CREATE OR REPLACE FUNCTION vodka_proc_order_status(
    IN in_w_id integer,
    IN in_d_id integer,
    INOUT in_c_id integer,
    IN in_c_last varchar(16),
...
BEGIN
    --If C_LAST is given instead of C_ID (60%), determine the C_ID.
    IF in_c_last IS NOT NULL THEN
		in_c_id = vodka_cid_from_clast(in_w_id, in_d_id, in_c_last);
    END IF;
...
CREATE OR REPLACE FUNCTION vodka_proc_stock_level(
    IN in_w_id integer,
    IN in_d_id integer,
...
CREATE OR REPLACE FUNCTION vodka_proc_delivery_bg(
	IN in_w_id integer,
