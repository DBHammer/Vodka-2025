...
public class HTAPCheckMetrics {
    ...
    public HTAPCheckMetrics(String resultDirectory) {
        ...
        try {
            ...
            runInfoCSV.write("taskType,txnCompleteTime,gapTime,startTime,endTime,tryNum,pass,isApConnErr,haTtrickFreshness,vodkaFreshness,syncLatency,executeLatecy,unifiedLatency\n");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void add(TaskResult taskResult) {
		...
		infoFmt.format("%d,%d,%f,%d,%d,%d,%s,%s,%d,%d,%d,%d,%d\n",
						taskResult.taskType,
                        ...
                        taskResult.haTtrickFreshness,
                        taskResult.vodkaFreshness,
                        taskResult.syncLatency,
                        taskResult.executeLatecy,
                        taskResult.unifiedLatency);
        try {
            lock.lock();
