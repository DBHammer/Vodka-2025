...
public class LoadDataWorker implements Runnable {
    ...
    LoadDataWorker(int worker, Connection dbConn, BasicRandom rnd)
        ...
        // " ol_shipinstruct, ol_receipdate, ol_commitdate, ol_returnflag, ol_suppkey,
        // current_ts) "
        // +
        // "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        stmtOrderLine = dbConn.prepareStatement(
        ...
        try {
            for (int i = 1; i <= SF * 10000; i++) {
                ...
                if (OLTPClient.isSkewDist == false) {
                    ...
                    stmtSupplier.setInt(2, SkewRandom.next("vodka_supplier.s_nationkey", 25));
                }
        ...
        for (i_id = 1; i_id <= 100000; i_id++) {
            ...
            if (writeCSV) {
                ...
                if (OLTPClient.isSkewDist == false) {
                    ...
                    int k = SkewRandom.next("vodka_item.i_container", BasicRandom.CONTAINERS.length);
                    stmtItem.setString(6, BasicRandom.CONTAINERS[k]);
                    int sz = 1 + SkewRandom.next("vodka_item.i_size", 50);
                    stmtItem.setInt(7, sz);
                    int b = 1 + SkewRandom.next("vodka_item.i_brand", 5);
                    stmtItem.setString(8, "Brand#" + b + mfgr);
                    k = SkewRandom.next("vodka_item.i_type", BasicRandom.ITEM_TYPES.length);
                    stmtItem.setString(9, BasicRandom.ITEM_TYPES[k]);
                }
        ...
        for (int s_i_id = 1; s_i_id <= 100000; s_i_id++) {
            ...
            if (writeCSV) {
                ...
                if (OLTPClient.isSkewDist == false) {
                    ...
                    double c = 1 + SkewRandom.next("vodka_stock.s_supplycost", 1000);
                    stmtStock.setDouble(18, c);
                }
                stmtStock.setInt(19, w_id * s_i_id % 10000 + 1);
                // stmtStock.setInt(19, (s_i_id + rnd.nextInt(0, 3) * ((w_id * 10000) / 4 +
                // (s_i_id - 1) / (w_id * 10000)))
                // % (w_id * 10000) + 1);
        ...
        for (int d_id = 1; d_id <= 10; d_id++) {
            ...
            for (int c_id = 1; c_id <= 3000; c_id++) {
                if (writeCSV) {
                    ...
                    if (OLTPClient.isSkewDist == false) {
                        ...
                        stmtCustomer.setString(12, rnd.getNString(16, 16)); // c_phone
                        stmtCustomer.setString(22, rnd.getCktSegement(0, 4)); // c_mktsegment
                    } else {
                        stmtCustomer.setInt(10, SkewRandom.next("vodka_customer.n_nationkey", 25));
                        int pre = SkewRandom.next("vodka_customer.c_phone", 100);
                        String tail = SkewRandom.uniformLong(0, 1_00_0000_0000_0000L) + "";
                        tail = "0".repeat(14 - tail.length()) + tail;
                        stmtCustomer.setString(12, String.format("%02d%s", pre, tail));
                        int k = SkewRandom.next("vodka_customer.c_mktsegment", 5);
                        stmtCustomer.setString(22, BasicRandom.cktSegment[k]);
                    }
            ...
            for (int o_id = 1; o_id <= 3000; o_id++) {
                ...
                if (writeCSV) {
                    ...
                    String word1 = BasicRandom.SUBST_WORD1[rnd.nextInt(BasicRandom.SUBST_WORD1.length)];
                    String word2 = BasicRandom.SUBST_WORD2[rnd.nextInt(BasicRandom.SUBST_WORD2.length)];
                    stmtOrder.setString(9, word1 + " " + word2);
                    ...
                    if (writeCSV) {
                        ...
                        if (o_id < 2101) {
                            ...
                            if (OLTPClient.isSkewDist == false) {
                                ...
                                stmtOrderLine.setString(16, r == 0 ? "R" : r == 1 ? "A" : "N");
                                int k = SkewRandom.next("vodka_order_line.ol_shipmode", 7);
                                stmtOrderLine.setString(12, BasicRandom.sMode[k]);
                                k = SkewRandom.next("vodka_order_line.ol_shipinstruct", 4);
                                stmtOrderLine.setString(13, BasicRandom.sInstruct[k]);
                            }
                        } else {
                        ...
                        if (OLTPClient.isSkewDist == false) {
                            stmtOrderLine.setInt(8, rnd.nextInt(1, 50));
                            stmtOrderLine.setDouble(11, ((double) rnd.nextLong(0, 10)) / 100.00);
                        } else {
                            int q = 1 + SkewRandom.next("vodka_order_line.ol_quantity", 50);
                            stmtOrderLine.setInt(8, q);
                            double d = SkewRandom.next("vodka_order_line.ol_discount", 10) / 100.0;
                            stmtOrderLine.setDouble(11, d);
                        }
                        // 11. l_discount
                        stmtOrderLine.setInt(17, w_id * ol_i_id % 10000 + 1);
                        // stmtOrderLine.setInt(17,
                        // (ol_i_id + rnd.nextInt(0, 3) * (w_id * 10000 / 4 + (ol_i_id - 1) / (w_id *
                        // 10000)))
                        // % (w_id * 10000) + 1); // ol_suppkey按tpch计算方式得到

                        // if (!OLTPClient.isOnlineDDL) {
                        // stmtOrderLine.setDouble(18, rnd.nextDouble(0, 0.08)); // 12. l_tax's range:
                        // [0, 0.08]
                        stmtOrderLine.setDouble(18, rnd.nextDouble(0, 0.08)); // 12. l_tax's 0.1 when ddl
                        long nowMs = System.currentTimeMillis();
                        Timestamp ts = new Timestamp(nowMs);
                        ts.setNanos((int) (nowMs % 1000) * 1_000_000);
                        stmtOrderLine.setTimestamp(19, ts);
                        // } else {
                        // stmtOrderLine.setTimestamp(18,
                        // Timestamp.valueOf(simFormat.format(System.currentTimeMillis())));
                        // // }
